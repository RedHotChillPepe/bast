...
const RegisterPage = ({ route }) => {
  ...
  const handlePasswordChange = (field, value) => {
    if (!allowedRegex.test(value)) {
      setFormErrors((prev) => ({
        ...prev,
        [field]: "Введён недопустимый символ",
      }));
      return;
    }
    ...
    setFormData((prevData) => {
      ...
      const confirmPassword =
        field === "confirmPassword" ? value : prevData.confirmPassword;
      ...
      >
        <TouchableOpacity
          ...
          style={[
            ...
            { borderColor: formData[typeCheck] ? "#2C88EC" : "#A1A1A1" },
          ]}
        >
          {formData[typeCheck] && <View style={styles.radio__point}></View>}
        </TouchableOpacity>
        <View style={styles.radio__text__container}>
          ...
          <TouchableOpacity>
            <Text style={[styles.radio__text, { color: "#2C88EC" }]}>
    ...
    return (
      <View style={{ marginTop: 40, alignSelf: "stretch" }}>
        <Text style={[styles.confirm__text, { color: "#808080" }]}>
          Нажимая «Далее», вы соглашаетесь с условиями следующих
          документов:&nbsp;
    ...
    return (
      <TouchableOpacity
        style={[
          styles.submitButton,
          {
            backgroundColor: hasFormErrors() ? "#9bbfea" : theme.colors.accent,
          },
        ]}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {renderInput()}
      {renderSubmitButton()}
      <Modal visible={isShowDocumentModal}>
        <DocumentationScreen
          selectedDocument={selectedDocument}
          handleClose={() => setIsShowDocumentModal(false)}
        />
      </Modal>
    </SafeAreaView>
  );
