...
const ServicesPage = ({ handleClose, selectedItem }) => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                setFormData((prev) => ({ ...prev, user_name: result.name ?? "", user_surname: result.surname ?? "", user_phone: formatPhoneNumber(result.phone) ?? "", prevPhone: result.phone ?? "" }))
            } catch (error) {
    ...
    const listInputs = [
        ...
        { title: "Фамилия", valueName: "user_surname", placeholder: "Иванов", type: "" },
        { title: "Контактный телефон", valueName: "user_phone", placeholder: " + 7(XXX) XXX- XX - XX", type: "tel" },
    ]

    const formatPhoneNumber = (text) => {
        if (!text) return "";
        if (formData.prevPhone && formData.prevPhone.length > text.length) return text;
    ...
    const handleChangePhone = (field, value) => {
        ...
        setFormData((prevData) => ({ ...prevData, [field]: formatted, prevPhone: formatted }));
    ...
    const renderInputs = () => {
        return <Animated.View style={{ transform: [{ translateX }], marginTop: 19, gap: 16 }}>
            {isShowInputs ?
                listInputs.map((item, index) =>
                    <View key={index} style={[theme.spacing.small, { gap: theme.spacing.small }]}>
                        ...
                        {formErrors[item.valueName].length > 0
                            && <Text Text style={[theme.typography.regular("error"), { textAlign: "left" }]}>
                                {formErrors[item.valueName]}
                            </Text>}
                    </View>
                ) :
                <Text style={[theme.typography.regular(), { textAlign: "left" }]}>{selectedItem.subText}{"\n"}
                    {"\n"}Чтобы подать заявку, оставьте свои контактные данные.</Text>}
            {(isShowInputs && formErrors.message.length > 0 && !successMessage.length) &&
                <Text Text style={[theme.typography.regular("error"), { textAlign: "left" }]}>
                    {formErrors.message}
                </Text>}
            {isShowInputs && successMessage.length > 0 && <Text style={[theme.typography.regular("success"), { textAlign: "left" }]}>{successMessage}</Text>}
        </Animated.View>
    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await createServiceRequest({ ...formData, user_phone: normalPhoneNumber });
            if (!result.success) throw result

            setSuccessMessage(result.message)
            // handleClose();
        } catch (error) {
            setFormErrors((prev) => ({ ...prev, message: error.message || "Произошла ошибка, повторите попытку позже" }));
        }
    }
    ...
    return (
        <SafeAreaView style={theme.container}>
            <UniversalHeader title={isShowInputs ? "Создание заявки" : selectedItem.title} handleClose={customHandleClose} />
            <View style={{ flex: 1, marginTop: 8 }}>
                <Text style={[theme.typography.title2, { textAlign: "left" }]}>{selectedItem.subTitle}</Text>
                {renderInputs()}
            </View>
            <TouchableOpacity
                style={[theme.buttons.classisButton, { marginBottom: 28, opacity: isShowInputs && hasFormErrors() ? 0.6 : 1 }]}
                disabled={isShowInputs && hasFormErrors()}
                onPress={handleSubmit}
            >
                <Text style={theme.typography.buttonTextXL}>{isShowInputs ? "Отправить заявку" : "К заявке"}</Text>
            </TouchableOpacity>
        </SafeAreaView>
    )
}
