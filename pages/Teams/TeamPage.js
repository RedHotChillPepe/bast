...
const TeamPage = (props) => {
    ...
    const [activeExitRequestCurrentUser, setActiveExitRequestCurrentUser] = useState(null);
    ...
    const handleRejectRequest = async (requestId) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setRequestTeam(prevRequests => prevRequests.filter(request => request.request_id !== requestId));
            } else {
            ...
            <View style={{ flexDirection: "row", columnGap: 8 }}>
                ...
                <View style={styles.item__about}>
                    <View style={styles.people}>
                        ...
                        <Text style={styles.people__classification}>{member.role_description}</Text>
                    </View>
                    <View style={styles.status_container}>
                        {((isOwner || currentUser.id == member.user.id &&
                            currentUser.usertype == member.user.usertype) && selectedList !== "request") ?
                            <View style={{ flexDirection: 'row' }}>
                                <Text style={styles.people__status}>в работе: </Text>
                                <Text style={styles.people__status__number}>{member.request_count}</Text>
                            </View> :
                            <View style={{ flexDirection: 'row' }}>
                                <Text style={styles.people__status}>{member.type_description}</Text>
                            </View>
                        }
                    </View>
                </View>
    ...
    const renderPeoples = () => {
        ...
        const sortedMembers = [...teamMembers].sort((a, b) => {
            const aIsCurrent = a.user.id === currentUser.id && a.user.usertype === currentUser.usertype;
            const bIsCurrent = b.user.id === currentUser.id && b.user.usertype === currentUser.usertype;
        ...
        return (
            <View style={styles.containerItem}>
                {sortedMembers.map((member) =>
                    <Pressable key={`member-${member.member_id}`} onPress={() => handleSelectedPeople(member)}>
                        {member.role !== 1 &&
                            <View style={styles.item}>
                                {renderPeopleItem(member)}
                            </View>
                        }
                    </Pressable>
                )}
    ...
    const renderRequest = () => {
        if (requestTeam.length === 0) return <Text style={styles.text__empty}>Нет заявок</Text>

        return (<View style={styles.containerItem}>
            {requestTeam.map((request) =>
                <Pressable key={`request-${request.request_id}`} onPress={() => handleSelectedPeople(request)}>
                    <View style={styles.item}>
                        ...
                        <View style={{ flexDirection: "row", columnGap: 8, }}>
                            ...
                            <Pressable
                                ...
                                style={[styles.request__button, { backgroundColor: "#2C88EC" }]}
                            >
    ...
    const handleCancelLeave = async () => {
        ...
        setError("");
        try {
            const result = await rejectTeamRequest(activeExitRequestCurrentUser.request_id);
            if (result.success) {
                setIsShowLeaveTeamModal(false);
        ...
        return (
            <View style={styles.modalOverlay}>
                <View style={styles.modalContainer}>
                    ...
                    {isLoadingResult ? <View style={{ paddingVertical: 24 }}><ActivityIndicator size="large" color="#32322C" /></View> :
                        <View>
                            <Text style={[styles.modalSubtitle, { color: "#3E3E3E" }]}>
                                {hasPendingRequest ? (
                                    <>
                                        Вы ранее отправили заявку на выход из команды{' '}
                                        <Text style={{ fontWeight: 'bold' }}>{selectedTeam.team_name}</Text>. Хотите её отменить?
                                    </>
                                ) : (
                                    <>
                                        Вы уверены, что хотите покинуть команду{' '}
                                        <Text style={{ fontWeight: 'bold' }}>{selectedTeam.team_name}</Text>?
                                    </>
                                )}
                            </Text>
                            {error ? (
                    ...
                    <View style={styles.modalButtons}>
                        ...
                        >
                            <Text style={[styles.cancelButtonText, { color: isLoadingResult ? "#3E3E3E66" : '#3E3E3E' }]}>{hasPendingRequest ? "Оставить" : "Отменить"}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
    ...
    const listModals = [
        {
            ...
            content: <TeamInvationPage handleClose={() => setIsShowInvationModal(false)} teamData={selectedTeam} />,
        },
        {
    ...
    const handleSelectedPeople = (people) => {
        const isSelf = currentUser?.id === people?.user?.id && currentUser?.usertype === people?.user?.usertype;
