...
const UserTeamsPage = (props) => {
  ...
  const renderTeams = () => {
    if (teamsData.length == 0)
      return (
        <Text style={styles.text__empty}>У вас нет действующих команд</Text>
      );
    return (
      <View style={styles.containerItem}>
        {teamsData.map((item) => (
          <Pressable
            key={item.team_id}
            onPress={() => handleSelectedTeam(item)}
          >
                ...
                <View style={styles.team__about}>
                  ...
                  {item.member_role === 1 && (
                    <View style={styles.status_container}>
                      ...
                      <Text style={styles.team__status__number}>
                        {item.team_request_count}
                      </Text>
                    </View>
                  )}
                </View>
                <Text style={styles.team__classification}>
                  {item.description}
                </Text>
              </View>
            </View>
