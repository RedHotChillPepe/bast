...
export default function CreateTeamPage(props) {
  ...
  const inputList = [
    ...
    {
      text: "Описание",
      placeholder: "Работа с ипотечными клиентами",
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      navigator.navigate("Error", {
        messageProp: "Произошла ошибка при создании команды",
      });
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>      
      <KeyboardAvoidingView style={Platform.OS !== "ios" ? { flex: 1 } : styles.container}>
        {renderHeader()}
        <View style={styles.containerItem}>{renderInput()}</View>
        <Pressable
          style={[
            styles.button,
            {
              backgroundColor: isFormValidAndChanged()
                ? "#2C88EC"
                : "#2C88EC66",
            },
          ]}
        ...
        <Modal visible={isShowInvationModal}>
          <TeamInvationPage
