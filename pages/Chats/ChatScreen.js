...
const ChatScreen = (props) => {
    ...
    const { socket, isConnected, sendMessage, emitTyping, emitStopTyping, markAsRead } = useChatSocket(
        selectedChat?.id,
        currentUser,
        handleIncoming,
        handleTyping,
    ...
    useEffect(() => {
        ...
        const handleMessagesRead = ({ result }) => {
            ...
            setMessages(prev =>
                prev.map(msg =>
                    ids.includes(msg.id) ? { ...msg, status: 1 } : msg
                )
            );
        };
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        getChatMessages(selectedChat.id).then(res => {
        ...
        return (
            <Pressable
                ...
                <View style={styles.post__info}>
                    ...
                    <Text style={styles.info__location}>{post.city}, {post.full_address}</Text>
    ...
    const renderItem = ({ item }) => {
        if (item.type === 'header') {
            return (
                <Text style={styles.full__date}>{item.date}</Text>
            );
        }
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    {item.isMyMessage && <CheckAllIcon color={item.isRead ? "#2C88EC" : "#8E8E93"} />}
                </View>
            </View>
        );
    };
