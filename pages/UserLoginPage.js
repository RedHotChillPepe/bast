...
const UserLoginPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.tempToken !== undefined) {
        navigation.dispatch(
          CommonActions.reset({
            index: 0,
            routes: [
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          style={{ marginTop: 16 }}
        >
          <Text style={styles.reset__text}>
            {isResetPassword ? "Я вспомнил пароль" : "Забыли пароль?"}
          </Text>
        </TouchableOpacity>
      </View>
    );
  ...
  const renderSubmitButton = () => {
    return (
      <TouchableOpacity
        style={[styles.button, { opacity: hasFormErrors() ? 0.5 : 1 }]}
        disabled={hasFormErrors()}
        onPress={() =>
          isResetPassword ? handleResetPassword() : handleSubmit()
        }
      >
