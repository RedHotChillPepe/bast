...
export default function CreateHousePostPage({ navigation }) {
  ...
  const AboutHouse = () => {
    const otherItems = inputListHouseinfo.slice(0, inputListHouseinfo.length - 4);
  ...
  const inputListConstruction = [
    ...
    { text: "Перегородки", placeholder: "Материал", valueName: "partitionMaterial", },
  ...
  const inputListCommunications = [
    { text: "Тариф на электроэнергию", placeholder: "Выбрать тариф", valueName: "electricity", },
    { text: "Водоснабжение", placeholder: "Тип водоснабжения", valueName: "water", },
    { text: "Газ", placeholder: "Подключение", valueName: "gas", },
    { text: "Отопление", placeholder: "Тип системы отопления", valueName: "heating", },
    { text: "Канализация", placeholder: "Тип системы канализации", valueName: "sewerege", },
  ]
  ...
  const getPage = () => {
    switch (page) {
      case 0:
        return <InputImage setFormData={setFormData} photos={formData.photos} />;
      case 1:
