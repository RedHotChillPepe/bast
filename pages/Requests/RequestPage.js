...
const RequestPage = (props) => {
    const { handleClose, setTeamMembers, prevStatusRef, setSelectedRequest, selectedRequest, setRequests, stages, user, selectedTeam, setSelectedPeople, isDeal = false, isOwner = false } = props;
    ...
    useEffect(() => {
        ...
        if (status.id === -1) {
            ...
            const idx = typeof fromRef === 'number'
                ? fromRef
    ...
    const renderHeader = () => {
        return <View style={styles.header}>
            ...
            <Text style={styles.header__title}>{isDeal ? currentStage == maxStage ? "Выплата агенту" : "Сделка" : "Заявка"} {(!isDeal || currentStage !== maxStage) && selectedRequest.request_id || selectedRequest.last_assign.id}</Text>
            <Pressable>
                <ShareIcon />
            </Pressable>
        </View>
    ...
    const getCurrentStatus = () => {
        return stages.find(stage => stage.status_id === currentStage) || stages[0];
    };
    ...
    const renderStatus = () => {
        ...
        return (
            <View style={[styles.status__flag, { backgroundColor: isRejection ? "#FF2D55" : statusId == 4 ? theme.colors.success : statusId == 0 ? '#FFC107' : "#2C88EC" }]}>
        ...
        return <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <View style={{ flexDirection: "row" }}>
                ...
                <Text style={styles.update__date}>
                    {new Date(selectedRequest.last_assign.assigned_at).toLocaleDateString("ru-RU", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                </Text>
            </View>
            {renderStatus()}
        </View>
    ...
    const renderRequestBody = () => {
        return <View style={{ rowGap: 16, marginTop: 8 }}>
            <View style={{ rowGap: 4 }}>
                <Text style={styles.initials}>{selectedRequest.user.name} {selectedRequest.user.surname}</Text>
            ...
            <View style={[styles.item, { rowGap: 32 }]}>
                <View style={{ rowGap: 16 }}>
                    ...
                    <Text style={styles.stage__description}>
                        {isRejection ? stages[0].status_description : currentStatus.status_description}
                    </Text>
                </View>
                {errorMessage && <Text style={[theme.typography.errorText, { textAlign: "center" }]}>{errorMessage}</Text>}
                {(((currentStage < maxStage && user.usertype === 3) || isDeal && maxStage == currentStage) && !isOwner) &&
                    renderStageButton()
                }
            </View>
        );
    ...
    const handleChangeAssign = async (people) => {
        try {
            ...
            setSelectedPeople(prev => ({
                ...
                requests: prev?.requests?.filter(item => item.id !== selectedRequest.last_assign.id) || []
            }));
            ...
            setTeamMembers((prevMembers) => {
                ...
                const updatedMembers = prevMembers.map((member) => {
                    const requestIndex = member.requests?.findIndex((r) => r.id === requestId) ?? -1;
                    ...
                    if (requestIndex !== -1) {
                        ...
                        return {
                            ...
                            request_count: String(Math.max(0, Number(member.request_count) - 1)),
                        };
                    }
    ...
    const handleChangeStage = async (isSetRejection = false) => {
        try {
            ...
            const isRestore =
                ...
                (now - new Date(last_assign.assigned_at).getTime()) <= 7 * 24 * 60 * 60 * 1000;
            ...
            if (isRestore) {
                ...
                const updatedLast = {
                    ...
                    status: {
                        id: -1,
                        description: stages.find(s => s.status_id === -1).status_description,
            ...
            await changeStatusRequest(selectedRequest.request_id || last_assign.id, newStatus);
            ...
            if (setRequests)
                ...
                setSelectedPeople(prev => {
                    if (newStatus !== -1 || newStatus !== 4) prev.request_count = Math.max(0, Number(prev?.request_count || 0) - 1);
                    console.log(prevStatusRef);

                    const requestIndex = prev.requests?.findIndex((r) => r.id == selectedRequest.last_assign.id) ?? -1;
                    if (requestIndex !== -1) {
                ...
                setSelectedRequest(prev => {
                    console.log(prev);
                    return { ...prev, assigned_at: new Date() }
                })
            }
    ...
    const renderStageButton = () => {
        if (isLoading) return <View style={{ alignItems: "center", justifyContent: "center" }}><ActivityIndicator size={"large"} /></View>

        if (isRejection) return <View style={styles.stage__button__container}>
            <TouchableOpacity
            ...
            >
                <Text
                    ...
                    style={[styles.stage__button__text, { color: "#F2F2F7" }]}>Возобновить</Text>
            </TouchableOpacity>
        </View>
        ...
        return <View style={styles.stage__button__container}>
            {!isDeal &&
                ...
                >
                    <Text style={[styles.stage__button__text, { color: "#2C88EC" }]}>{stages[0].action_text}</Text>
                </TouchableOpacity>
            }
            ...
            <TouchableOpacity
                ...
                style={[styles.stage__button, { backgroundColor: `#2C88EC${isDeal && maxStage == currentStage && !documentIsUpload ? '60' : ''}` }]}
                onPress={isDeal && maxStage == currentStage ? listPaymentsButton[paymentsStage].handlePress : () => handleChangeStage(false)}
            >
                <Text
                    numberOfLines={1}
                    adjustsFontSizeToFit
                    style={[styles.stage__button__text, { color: "#F2F2F7" }]}>{isDeal && maxStage == currentStage ? listPaymentsButton[paymentsStage].title : stages[currentStage + 1].action_text}</Text>
            </TouchableOpacity>
        </View>
    }
        ...
        >
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: theme.spacing.small }}>
                    <CreditCardIcon />
                    <Text style={theme.typography.regular('text')}>{"2200 4480 7700 7654"}</Text>
                </View>
                <CopyIcon size={24} strokeWidth={2} />
            </View>
            <Text style={theme.typography.regular('caption')}>Переведите вознаграждение по номеру банковской карты</Text>
        </TouchableOpacity>
    }
    ...
    return (
        <View style={styles.container}>
            ...
            {(user.usertype === 3 || user.usertype === 2 && currentStage < maxStage) &&
                <TouchableOpacity
                    disabled={(currentStage == maxStage || isRejection) || (isDeal && !documentIsUpload)}
                    style={[styles.button, { backgroundColor: !isDeal && (isOwner && currentStage !== maxStage && !isRejection) || (isDeal && documentIsUpload) ? "#2C88EC" : "#2C88EC66" }]}
                    onPress={() => !isOwner ? handleClose() : setIsShowSelectedPeople(true)}
                >
                    <Text style={styles.button__text}>{!isOwner ? "Закрыть сделку" : currentStage === 0 ? "Выбрать исполнителя" : "Сменить исполнителя"}</Text>
                </TouchableOpacity>
            }
            <Modal visible={isShowSelectedPeople} onRequestClose={() => setIsShowSelectedPeople(false)} animationType='slide'>
                <TeamPage selectedTeam={selectedTeam} handleChangeAssign={handleChangeAssign} isChangeAssign={isOwner} currentUser={user} handleClose={() => setIsShowSelectedPeople(false)} />
            </Modal>
        </View >
    )
}
