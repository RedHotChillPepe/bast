...
import { ActivityIndicator, FlatList, Image, Modal, Pressable, RefreshControl, SafeAreaView, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
const ReferralListPage = (props) => {
    ...
    const fetchReferrals = async () => {
        try {
            ...
            console.error(error);
            navigation.navigate("Error", { messageProp: "Ошибка при получении рефералов" });
        } finally {
            setIsLoading(false);
            setRefreshing(false);
        }
    ...
    const renderPeopleBlock = () => {
        return <View style={{ flex: 1 }}>
            <Text style={theme.typography.title3('text', 'start')}>Мои рефералы</Text>
            {isLoading ? <View style={{ marginTop: theme.spacing.medium }}><ActivityIndicator /></View> :
                <FlatList
                    data={referrals}
                    ...
                    ListEmptyComponent={<Text style={theme.typography.caption}>У вас нет рефералов</Text>}
                />
            }
        </View>
    }
        ...
        >
            ...
            <View style={{ gap: 6, flex: 1 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    <Text style={theme.typography.regularBold}>{item.name} {item.surname}</Text>
                    {/* <Text style={[theme.typography.statusText, { color: theme.colors.accent + 60 }]}>Завершена</Text> */}
                </View>
                <Text style={theme.typography.regular('caption', 400, 'start')}>{item.role}</Text>
            </View>
        </TouchableOpacity >
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            {renderCreditBLock()}
            {renderPeopleBlock()}
            <Modal visible={isShowModalQrCode}><TeamInvationPage handleClose={() => setIsShowModalQrCode(false)} isReferral={true} /></Modal>
            <Modal animationType='slide' visible={IsShowEditCredit}><EditCreditPage handleClose={handleCloseEdit} formData={formData} setCardNumber={setCardNumber} setFormData={setFormData} /></Modal>
            <CustomModal isVisible={showModalUser} onClose={() => setShowModalUser(false)} buttonLeft={<ChevronLeft />} buttonRight={<ShareIcon />}>
                {/*  */}
